...
def test_stylesheet_hard_codes_no_width_wider_than_a_phone() -> None:
    ...
    # `30rem` is table.cvs, which is meant to scroll sideways. `34rem` is the
    # floor that keeps a dense table's columns legible on a desktop; it is
    # allowed only because the phone block resets it, which the next assertion
    # checks -- a floor that survives to 320px is a horizontal scroll waiting
    # to happen.
    deliberate = {"30rem", "34rem"}
    ...
    phone = css[css.index("@media (max-width: 40rem)") :]
    assert ".table-wrap > table { min-width: 0; }" in phone, (
        "the desktop table floor is never reset, so it applies at 320px too"
    )
